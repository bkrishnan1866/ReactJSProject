...
import { getNamesAndLocation, getShipmentDetails, updateShipment } from './shipmentAPI';
import 'date-fns';
...
function ShipmentEditForm() {
    const { getCart } = useContext(ShipmentCartContext);
    ...
    const getShipmentDetailsFromPAI = async (shipmentId) => {
        let nameLocList = await getShipmentDetails(shipmentId);
        setShipmentDetails(nameLocList.shipment[0]);
    ...
    const onSubmit = async () => {
        const resp = await updateShipment(shipmentDetails);
        alert(resp.Message);
    }
    ...
    useEffect(() => {
        getShipmentDetailsFromPAI(params.id);
    }, [params]);

    return (
        <div>
            <label>Sent By : </label>
            <select name="s_shipment_sent_by" onChange={onSelect} value={shipmentDetails.s_shipment_sent_by} >
            ...
            <br />
            <label>Sent To : </label>
            <select name="s_shipment_sent_to" onChange={onSelect} value={shipmentDetails.s_shipment_sent_to} >
            <option value="" key="0"> SELECT Name </option>
            {
                shipmentSelectList.map((item, index) => {
                return (<option key={item.name + index} value={item.name}>
            ...
            <select name="s_shipment_loc_from" onChange={onSelect} value={shipmentDetails.s_shipment_loc_from} >
            ...
            <select name="s_shipment_loc_to" onChange={onSelect} value={shipmentDetails.s_shipment_loc_to} >
            <option value="" key="0"> SELECT Location </option>
            {
                shipmentSelectList.map((item, index) => {
                return (<option key={item.location + index} value={item.location}>
            ...
            <input type="text"
                ...
                name="s_tracking_num"
                value={shipmentDetails.s_tracking_num}
            ...
            <input type="text"
                ...
                value={shipmentDetails.d_shipment_cost}
                onChange={onChangeHandle}
            />
            <br></br>
            <label>Shipment Status : </label>
            <select name="b_shipment_status" onChange={onSelect} value={shipmentDetails.b_shipment_status} >
                <option value="" key="0"> SELECT Status </option>
                <option value="No" key="1"> No </option>
                <option value="Yes" key="2"> Yes </option>
            </select>
            <br /><br />
            <button onClick={onSubmit}> Update Shipment</button>
        </div>
    )
}
